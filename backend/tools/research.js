...
8. 搜索纪律：用 search 工具检索，一次一个意图；知识/信息类查询优先英文（英文语料质量更高）；搜不到就换表述或换 domain 重试一次；禁止凭知识编造来源或 URL。当需要高信息密度、带个人经验/时效性的内容时（方法论、选品、实操、行业判断），用 factreach 工具补一手信源：YouTube 字幕/无字幕音频转写、小宇宙播客转写、抖音口播、B站、HN/arXiv/Stack Overflow 等；知乎可经 factreach web read/Jina 读题面（完整回答需登录态）。此类口播/播客来源仍按一手/二手口径标注，并写明个人经验的时效性局限。
...
1. 先用 search 工具做 1-2 次广搜收集素材（不挑食，混沌输入），从素材中识别该领域的常见说法、变量、候选方法。若主题是方法论/选品/实操经验类，同时用 factreach 拉 1-2 条高密度一手口播源（YouTube 字幕/小宇宙播客/抖音），用于发现变量和候选方法。
