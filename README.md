...
### 有审美
空会话时是一片会躲开鼠标的鸟群（Vanta BIRDS 的原生复刻，独立仓库 [birds-effect](https://github.com/yipengfeifei/birds-effect)）；开始对话后换成 88 个真实星座 + 519 颗真实星等星色的星空。没有默认 UI 模板的味道——每处动效都逐个调过、不对就重做。

### 有人文气息
Agent 的过程是**给人看的，不是黑箱**：规划拆成节点图、研究的每一轮、每个工具在做什么，都如实摊在对话里（可折叠，不占屏）。产物能不能打开、失败的具体原因（「文件已不存在」而不是「读取超时」），都会明确告知，不静默吞掉。
