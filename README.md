...
### 提示词极简，能力做成工具
**整个 system prompt 实测 1,075 字符**——其中 683 是自定义 persona、174 是项目规则（AGENTS.md），其余是 SDK 的固定包装。作为对照：它替换掉的 pi 默认模板约 2,577 字符；技能清单也没有常驻（常驻要多花 5,519 字符），模型按 persona 里的指针自己 `ls` + `read` 去取。
...
**工具多，但管得住。** 常驻只有 13 个：pi 原生 7 个（`read`/`bash`/`edit`/`write`/`grep`/`find`/`ls`）+ 6 个支撑（`search`/`recall`/`update_plan`/`wait_for`/`subagent`/`load_toolkit`）。其余 10 个收在三个组里（`web` 5 / `research` 2 / `orchestrate` 3），**碰到对应场景才解锁**——它们的说明合计 479 字符，常驻时根本不进上下文，连用法索引都是解锁那一刻才注入。
