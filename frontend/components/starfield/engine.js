...
export function mountStarField({ root, promptEl: promptElIn, canvas: canvasIn, labelCanvas: labelCanvasIn, noteEl, failEl }) {
  ...
  /* 背景不需要 Retina 级锐度：dpr 2 → 1.5 可以把全屏片元数砍掉 44%，
     而全屏透镜 pass + 15000 个加法粒子的开销全部随 dpr² 走。 */
  ...
  /* 帧率：交互时 30fps，静止 4 秒后降到 12fps。星空是氛围，60fps 是纯浪费。 */
  const FPS_ACTIVE  = Number(new URLSearchParams(location.search).get('fps') ?? 30);
  const FPS_IDLE    = Number(new URLSearchParams(location.search).get('idlefps') ?? 12);
